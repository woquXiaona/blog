...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            var id = "${param.id}";
            //发送ajax请求携带id参数查询分类详情
            $.ajax({
                url: 'post/category/getById.do',
                data: {
                    "id": id
                },
                type: 'get',
                success: function (data) {
                    ...
                    var url = data.category.url;
                    $("#url").val(url.split(".")[0]);
                    $("#id").val(data.category.id);
                }
            })
            //保存修改分类
            $("#modifyClassifyBtn").on("click", function () {
                $("#modifyClassifyForm").ajaxForm({
                    success: function (data) {
                        if (data.success) {
                            window.location.href = "admin/modifyclassify.jsp?id=" + id;
                        } else {
                            alert("修改失败!")
                        }
                    },
        ...
        <div class="container-fluid">
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="admin/index.jsp">
                    <img alt="Catfish Management Center"
                         src="http://woquxiaona.com/public/common/images/catfish_white.png" width="20" height="20">
                </a>
                <a class="navbar-brand" href="admin/index.jsp">Catfish Blog管理中心</a>
            </div>
            <div class="collapse navbar-collapse">
                <ul class="nav navbar-nav">
                    ...
                    <li><a href="admin/change.html" title="修改登录密码"><span class="glyphicon glyphicon-lock"></span></a>
                    </li>
    ...
    <div class="row">
        ...
        <div class="col-xs-12 col-sm-9 col-md-10 pull-right" style="background-color:#FFFFFF;">
            ...
            <div class="container-fluid" style="min-height: 800px;">
                <div class="row">
                    <form id="modifyClassifyForm" name="modifyClassifyForm" method="post"
                          action="post/category/modifyClassify.do">
                        <h4>带&nbsp;<b><span class="text-danger">*</span></b>&nbsp;号的为必填项&nbsp;
                            <small>[<a
                                    href="admin/addclassify.jsp">添加新分类</a>]
                            </small>
                        </h4>
                        <div class="form-group">
                            <label>分类名称：&nbsp;<b><span class="text-danger">*</span></b></label>
                            <input id="name" type="text" class="form-control" name="name" placeholder="分类中文名称" required
                                   autofocus>
                            <input type="hidden" name="id" id="id">
                        </div>
                        <div class="form-group">
                            <label>英文名称:&nbsp;<b><span class="text-danger">*</span></b></label>
                            <input id="url" type="text" class="form-control" name="url" placeholder="分类英文名称" required/>
                        </div>
                        <hr>
                        <div class="text-center">
                            <button id="modifyClassifyBtn" type="submit" class="btn btn-default">保存
                                <span class="hidden">&nbsp;
                                    <img src="common/images/zhixing.gif" width="16" height="16"></span>
                            </button>
                        </div>
                    </form>
                </div>
            </div>
            <script src="common/js/modifyclassify.js"></script>
        </div>
    </div>
</div>
</body>
